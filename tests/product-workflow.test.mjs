...
test('artifact edit commits content and previous version without mutating the original', () => {
  ...
  const draft = structuredClone(artifact); draft.content.intro = '更新后的方案';
  const updated = commitArtifactEdit(artifact, draft, 'now');
  assert.equal(updated.content.intro, '更新后的方案');
  assert.notEqual(artifact.content.intro, updated.content.intro);
  assert.equal(updated.history[0].content.intro, artifact.content.intro);
