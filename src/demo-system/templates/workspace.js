import { ArtifactWorkbench } from '../components/artifacts.js?v=20260907f';
import { Composer } from '../components/composer.js?v=20260907f';
import { MessageFeed } from '../components/messages.js?v=20260907f';
import { ConversationHeader, TaskSidebar } from '../components/navigation.js?v=20260907f';
import { ConversationEditor } from '../components/conversation-editor.js?v=20260907f';
import { getActiveComposerQuestion, getFeedMessages } from '../conversation/component-registry.js?v=20260907f';
import { SkillChoices } from '../components/task-dialogs.js?v=20260907f';
...
export function WorkspaceTemplate(state) {
  ...
  const maximized = workbenchOpen && Boolean(state.artifactWorkspace.maximized);
  const mediaEditor = !maximized && workbenchOpen && (activeArtifact?.type === 'preview' || (activeArtifact?.type === 'video' && state.artifactWorkspace.activeView === 'edit'));
  const confirmation = getActiveComposerQuestion(state.messages);
  const feedMessages = getFeedMessages(state.messages);
  return `
    <div class="agent-shell ${workbenchOpen ? 'has-workbench' : ''} ${maximized ? 'workbench-maximized' : ''} ${mediaEditor ? `has-media-editor ${state.mediaTaskRailExpanded ? 'media-rail-expanded' : ''}` : ''} ${editorOpen ? 'is-editing' : ''} ${state.sidebarCollapsed ? 'sidebar-collapsed' : ''}">
      ...
      <section class="conversation-pane ${confirmation ? 'has-confirmation' : ''}">
        ...
        <main class="conversation-scroll" data-role="conversation-scroll" data-task-id="${escapeHtml(state.taskId || '')}"><div class="conversation-column">${MessageFeed({ messages: feedMessages, artifacts: state.artifacts, busy: state.busy })}</div></main>
