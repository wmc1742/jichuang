...
const bindings = {
  ...
  ProgressMessage: {
    controller: 'runAgentStage() → run.started / run.timer.tick',
    transport: 'SSE',
    endpoint: streamEndpoint,
    event: 'run.thinking.delta / run.timer.tick',
    trigger: 'Agent 收到用户消息后开始思考；此状态尚未调用 Skill 或 Tool',
    fields: [
      ['title', 'event.label', '缺省为“正在思考...”'],
      ['detail', 'event.elapsed_ms', '演示运行时按真实时间逐秒更新'],
      ['state', 'event.status', 'thinking'],
    ],
    sample: { type: 'run.timer.tick', run_id: 'run_02', status: 'thinking', label: '正在思考...', elapsed_ms: 4000 },
  },
  RunCompleteMessage: {
    ...
    endpoint: streamEndpoint,
    event: 'run.completed',
    trigger: '本轮思考或执行结束，同一个 Run 原位折叠，随后展示回复、追问或产物',
    fields: [
      ['detail', 'event.elapsed_ms', 'formatDuration()'],
      ['state', 'event.status', 'completed'],
    ],
    sample: { type: 'run.completed', run_id: 'run_02', status: 'completed', elapsed: '用时3m20s' },
  },
  ExecutionMessage: {
    controller: 'applyConversationEvent() → tool.started / run.step.updated',
    transport: 'SSE',
    endpoint: streamEndpoint,
    event: 'tool.started / skill.started / run.step.updated / run.completed',
    trigger: '同一 Run 实际开始调用 Skill 或 Tool 时，才从思考中原位切换为执行中',
    fields: [
    ...
    sample: { type: 'tool.started', run_id: 'run_03', tool: 'image.generate', label: '开始执行...', elapsed_ms: 3000 },
  },
  MultiSelectMessage: {
